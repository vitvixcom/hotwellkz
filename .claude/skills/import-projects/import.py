...
# Ручные переименования (по слагу) — для позиций без распознаваемого кода и т.п.
NAME_OVERRIDES = {"proekt-bani-anteplevo": "Проект бани «А-49»"}
...
def parse_csv(path):
    ...
    # ручные переименования имеют приоритет.
    for p in items:
        p["name"] = NAME_OVERRIDES.get(p["slug"], standardize_name(p))
